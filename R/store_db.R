...
store <- function(x, connection){
  ...
  dbWriteTable(connection, "element", new.element, append = TRUE)
  new.features <- x@Features %>%
    semi_join(new.element, by = c("hash" = "features"))
  dbWriteTable(new.features, conn = connection, name = "features", append = TRUE)
  new.feature <- x@Feature %>%
    semi_join(new.features, by = c("hash" = "feature"))
  dbWriteTable(new.feature, conn = connection, name = "feature", append = TRUE)
  x@Coordinates %>%
    semi_join(new.feature, by = "hash") %>%
    dbWriteTable(conn = connection, name = "coordinates", append = TRUE)
  x@Attribute %>%
    anti_join(
      dbReadTable(conn = connection, name = "attribute"),
      by = "id"
    ) %>%
    select_(~id, ~name, ~type) %>%
    dbWriteTable(conn = connection, name = "attribute", append = TRUE)
  attributevalue <- dbGetQuery(
    connection,
    "SELECT element, attribute, value, spawn, destroy FROM attributevalue WHERE destroy IS NULL"
  ) %>%
  ...
  attributevalue %>%
    ...
    dbWriteTable(conn = connection, name = "attributevalue", append = TRUE)
